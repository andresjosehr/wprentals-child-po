...
<html <?php language_attributes(); ?>>
    <head>
        ...
        wp_head(); 
        $favicon = esc_html(wprentals_get_option('wp_estate_favicon_image', 'url'));
    ...
    if ((get_post_type()== 'estate_property')||(get_post_type()== 'estate_shows')){
...
<?php
if (!is_search() && !is_tag() && !is_404() && !is_tax() && !is_category() && ( basename(get_page_template($post->ID)) === 'property_list_half.php' || get_post_type() === 'estate_property' || get_post_type() === 'estate_shows' )) {
    //do nothing for now  
...
if ((get_post_type() === 'estate_shows') && !is_tax() && !is_search()) {
    get_template_part('templates/artnevents/show_menu_hidden');
}
?>
...
if ($map_template === 1) {
    ...
    if (!is_404() && !is_tax() && !is_category() && !is_search() && !is_tag()) {
        if ( wpestate_check_if_admin_page($post->ID)) {
            ...
            if (('estate_property' == get_post_type())||('estate_shows' == get_post_type())) {
                if ( is_404()) {
