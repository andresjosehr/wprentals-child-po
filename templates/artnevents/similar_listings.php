...
$show_tax_artistic_discipline          = get_the_terms($post->ID, 'show_tax_artistic_discipline');
$show_tax_city   = get_the_terms($post->ID, 'show_tax_city');
...
if ($show_tax_artistic_discipline!=''):
    foreach ($show_tax_artistic_discipline as $item) {
        $items[] = $item->term_id;
    }
    $categ_array=array(
    ...
    if ($my_query->have_posts()) { ?>	
        <div class="similar_listings_wrapper">
            <div class="similar_listings">
                ...
                <h3 class="agent_listings_title_similar" ><?php esc_html_e('Similar Shows', 'wprentals'); ?></h3>   
                <?php
                $property_unit_slider= esc_html ( wprentals_get_option('wp_estate_prop_list_slider','') );    
    
                while ($my_query->have_posts()):$my_query->the_post();
                    get_template_part('templates/artnevents/show_unit');  
                endwhile;
                ?>
            </div>
        </div>
