...
<div class="col-md-12">
    ...
    <h4 class="user_dashboard_panel_title"><?php  esc_html_e('When is your show available','wprentals');?></h4>
    ...
    <!--         <p>
                <label for="property_icalendar_import"><?php esc_html_e('iCalendar import feed(feed will be read every 3 hours and when you hit save)','wprentals');?></label>
                <input type="text" id="property_icalendar_import" class="form-control" size="40" width="200" name="property_icalendar_import" value="<?php print $property_icalendar_import;?>">
                <a href="" id="delete_imported_dates" data-edit-id="<?php print $edit_id;?>"><?php esc_html_e('delete imported dates','wprentals');?></a>
            </p>
        -->
                ...
                if(is_array($property_icalendar_import_multi)){
                    foreach($property_icalendar_import_multi as $key=>$feed_data){
                        ...
                        <div class="icalfeed">
                            ...
                            <span class="delete_imported_dates_singular" data-edit-id="'.$edit_id.'" data-edit-id-key="'.$key.'">'.esc_html__('delete imported dates','wprentals').'</span>
                        </div>';
                    }
...
function wpestate_draw_reservation($reservation_note){
    ...
    if ( is_numeric($reservation_note)!=0){
        if($reservation_note>1505291773 ){
            return '<div class="rentals_reservation" >'.esc_html__('End Hour','wprentals').': '.$reservation_note.'/'.date('Y-m-d H:i',$reservation_note).'</div>';
        }else{
            return '<div class="rentals_reservation" >'.esc_html__('Booking id','wprentals').': '.$reservation_note.'</div>';
        }
    }else{
