...
?> 
    <div id="user_menu_open"> 
        <?php if($home_url!=$dash_profile){?>
                <a href="<?php print $dash_profile;?>" ><i class="fas fa-cog"></i><?php esc_html_e('My Profile','wprentals');?></a>   
        <?php   
        }
        ?>
        ...
        <?php if($home_url!=$dash_favorite){?>
            <a href="<?php print $dash_favorite;?>" class="active_fav"><i class="fas fa-heart"></i><?php esc_html_e('Favorites','wprentals');?></a>
        <?php   
        }
        
        if($home_url!=$dash_reservation  ){?>
            <a href="<?php print $dash_reservation;?>" class="active_fav"><i class="fas fa-folder-open"></i><?php esc_html_e('Reservations','wprentals');?></a>
        <?php   
        }
        
        if($home_url!=$dash_bookings && wpestate_check_user_level() ){?>
            <a href="<?php print $dash_bookings;?>" class="active_fav"><i class="far fa-folder-open"></i><?php esc_html_e('Bookings','wprentals');?></a>
        <?php   
        }
        if($home_url!=$dash_inbox){
             $no_unread=  intval(get_user_meta($userID,'unread_mess',true));?>
            <a href="<?php print $dash_inbox;?>" class="active_fav"><div class="unread_mess_wrap_menu"><?php print $no_unread;?></div><i class="fas fa-inbox"></i><?php esc_html_e('Inbox','wprentals');?></a>
        <?php   
         }
        if($home_url!=$dash_invoices && wpestate_check_user_level() ){?>
            <a href="<?php print $dash_invoices;?>" class="active_fav"><i class="far fa-file"></i><?php esc_html_e('Invoices','wprentals');?></a>
        <?php   
        }
        ?>
