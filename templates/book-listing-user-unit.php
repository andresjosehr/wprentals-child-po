...
$property_price     =   get_post_meta($booking_id, 'show_price', true);

//error_log("show_price: ". $property_price);

//error_log("The post: ".print_r(get_post_meta($post->ID), TRUE));
//error_log("The post: ".print_r(get_post_meta($booking_id), TRUE));
...
<div class="col-md-12 ">
    <div class="dasboard-prop-listing ">
        ...
        <div class="prop-info">
            ...
            <div class="user_dashboard_listed">
                <strong><?php esc_html_e('Invoice No: ','wprentals');?></strong> <span class="invoice_list_id"><?php print $invoice_no;?></span>   
            </div>

            <div class="user_dashboard_listed">
                <strong><?php esc_html_e('Members: ','wprentals');?> </strong> <?php print $number_members; ?>  
            </div>    

            <?php  if($to_be_paid>0 && $booking_status_full!='confirmed') { ?>
                <div class="user_dashboard_listed" style="color:red;">
                   <strong><?php esc_html_e('Balance: ','wprentals');?> </strong> <?php print $to_be_paid_show.' '.__('to be paid until ','wprentals').' '.wpestate_convert_dateformat_reverse($booking_from_date); ?>  
                </div> 
            <?php } ?>
            ...
            if($event_description!=''){
                print ' <div class="user_dashboard_listed event_desc"><strong>'.esc_html__( 'Comment: ','wprentals').'</strong>'.$event_description.'</div>';
            }
            ?>
        </div>
            ...
            if ($booking_status=='confirmed'){
                ...
                if(strtotime($booking_to_date) < time() ){
                    if ( get_post_meta($booking_id,'review_by_'.$userID,true) != 'has' ){
                        print '<span class="tag-post-review post_review" data-bookid="'.$post->ID.'" data-listing-review="'.$booking_id.'">'.esc_html__( 'Post Review','wprentals').'</span>';
                    }else{
