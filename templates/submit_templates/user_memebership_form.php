...
?> 
    ...
    if( $paid_submission_status == 'per listing'){
        ...
        print '<p class="full_form-nob">'.esc_html__(  'Price: ','wprentals').'<span class="submit-price">'.$price_submission.' '.$submission_curency_status.'</span></p>';
        print '<p class="full_form-nob">'.esc_html__(  'Featured (extra): ','wprentals').'<span class="submit-price">'.$price_featured_submission.' '.$submission_curency_status.'</span></p>';
        print'</div>'; // end submit container
     }
    ?>