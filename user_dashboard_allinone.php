    ...
    <div class=" dashboard-margin">
        ...
        <div class="row admin-list-wrapper inbox-wrapper">    
            ...
            <div class="arrow-wrapper-allinone_legend">
                ...
                <div class="calendar-pending calendar_pad has_future"></div>
                <div class="allinone_legend"><?php _e('Waiting for Approval','wprentals') ?></div>
    ...
    <div class="modal-dialog">
       <div class="modal-content allinone_modal">
            ...
            <div class="modal-header"> 
              ...
              <h2 class="modal-title_big"><?php esc_html_e('Set Period reservation','wprentals');?></h2>
              <h4 class="modal-title" id="myModalLabel"><?php esc_html_e('Mark dates as booked for selected period','wprentals');?></h4>
            </div>
            ...
            <div class="modal-body">
                ...
                <?php 
                error_log( print_r($submission_page_fields, TRUE) );
                ...
                <div class="col-md-12 clean_reservation">
                    ...
                    <input type="checkbox" id="block_dates" value="1"  checked="true" hidden>
                </div>  

                <div class="col-md-12 clean_reservation">
                    <label for="book_notes"><?php echo esc_html__( 'Book notes. ','wprentals');?></label>
                    <textarea id="book_notes" name="book_notes" cols="50" rows="6" class="form-control"></textarea>
                </div>  
                
                <button type="submit" id="allinone_set_custom_show" class="wpb_button  wpb_btn-info  wpb_regularsize   wpestate_vc_button  vc_button"><?php esc_html_e('Set booking period','wprentals');?></button>
...
function wpestate_draw_month_allinone($month_no, $unixmonth, $daywithpost,$thismonth,$thisyear,$last_day){
    ...
    $myweek = array();
        ...
        $user_agent_id = get_user_meta($current_user->ID, 'user_agent_id', true);
        ...
        if( !$prop_selection->have_posts() ){
            $calendar_output.= ' '.esc_html__( 'You don\'t have any shows yet!','wprentals').' ';
        }else{
            while ($prop_selection->have_posts()): $prop_selection->the_post();   

               // error_log("The post: ".print_r(get_post(), TRUE));

                if(get_post_status() == "publish"){

                    //error_log("post publish");

                    $post_id                    =   get_the_ID();

                    $link= esc_url ( get_permalink() );
                    $calendar_output.=  '<div class="show_tab_list_header"><a href="'.$link.'">';
                    $title=get_the_title();
                    ...
                    // error_log("post not publish");

                    $post_id                    =   get_the_ID();

                    $link= esc_url ( get_permalink() );
                    $calendar_output.=  '<div class="show_tab_list_header_pending"><a href="'.$link.'">';
                    $title=get_the_title();
                    $calendar_output .= mb_substr( html_entity_decode( $title ), 0, 20); 
       ...
       // error_log("Calendar Output: ".print_r($calendar_output, TRUE));
...
function wpestate_draw_reservation_allinone($reservation_note){
    ...
    if ( is_numeric($reservation_note)!=0){
        return '<div class="rentals_reservation allinone_reservation" data-internal-reservation="'.$reservation_note.'" >'.esc_html__('Booking id','wprentals').': '.$reservation_note.'</div>';
    }else{
