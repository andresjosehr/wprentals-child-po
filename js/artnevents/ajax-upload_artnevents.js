...
function delete_binder() {
    ...
    jQuery('#imagelist i.fa-trash-alt').click(function () {

        console.log("delete_binder");   

        var ajaxurl         =  ajax_vars.ajaxurl;

        var edit_id         =  jQuery('#listing_edit').val();
        ...
        var image = jQuery('#data_imageid').val();

       // console.log("jQuery(this).parent "+ JSON.stringify(jQuery(this).parent().attr('data-imageid')));

        // console.log("img_remove "+ img_remove);
        ...
        jQuery.ajax({
             ...
             url: ajaxurl,
             data: {
                 'action'            :   'wpestate_delete_file_show',
    ...
    array_cut=0;
        ...
        uploader.bind('FileUploaded', function (up, file, response) {
            ...
            if (result.success) {
                ...
                if (result.html !== '') {
                    $('#imagelist').append('<div class="uploaded_images" data-imageid="' + result.attach + '"><img src="' + result.html + '" alt="thumb" /><i class="far fa-trash-alt"></i> </div>');
                } else {
                    $('#imagelist').append('<div class="uploaded_images" data-imageid="' + result.attach + '"><img src="' + ajax_vars.path + '/img/pdf.png" alt="thumb" /><i class="far fa-trash-alt"></i> </div>');
                }
