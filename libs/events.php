...
add_action( 'wpestate_full_invoice_payment_reminder', 'wpestate_full_invoice_payment_reminder_function' );
...
function wpestate_full_invoice_payment_reminder_function(){
    ...
    while ($prop_selection->have_posts()): $prop_selection->the_post();      
        $post_id=get_the_iD();
        $booking_to_date    =  strtotime ( get_post_meta($post_id, 'booking_to_date', true) );
        print $post_id.' title '.get_the_title($post_id).'/ '.$booking_to_date.'</br>';
       
        if( ( time()+(60-60*24*3) ) > $booking_to_date){
