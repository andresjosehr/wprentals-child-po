...
function wpestate_single_listing_pins($prop_id){
    ...
    $counter=0;
    $unit                       =   wprentals_get_option('wp_estate_measure_sys','');
    $currency                   =   wprentals_get_option('wp_estate_currency_label_main','');
    $where_currency             =   wprentals_get_option('wp_estate_where_currency_symbol', '');
  ...
  //  set_time_limit (0);
    ...
    if( $markers === false ) {
        ...
        foreach ($prop_selection->posts as $key=>$value){
            $counter++;
            $markers[]=wpestate_pin_unit_creation( $value,$currency,$where_currency,$counter );  
        }
        if($with_cache==1){
...
function wpestate_pin_images(){
    ...
    if($pins===false){
        ...
        $pins['userpin']=esc_html( wprentals_get_option($name,'url') );  
