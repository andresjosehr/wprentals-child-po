...
function wpestate_category_labels_dropdowns($who){
...
function wpestate_show_extra_options_booking($post_id){
        ...
        if(is_array($extra_pay_options)){
            foreach($extra_pay_options as $key=>$extra_options){
                print'<div class="cost_row cost_row_extra wpestate_show_extra_options_booking"  data-value_add="'.$extra_options[1].'" data-value_how="'.$extra_options[2].'" data-value_name="'.$extra_options[0].'" >';
                    ...
                    print '<div class="cost_value"><div class="cost_value_show">'.  wpestate_show_price_booking($extra_options[1] ,$currency,$where_currency,1).'</div> '.wpestate_extra_options_exp($extra_options[2],$post_id).' </div>' ;
                print '</div>';
...
if( !function_exists('wpestate_show_stripe_form_upgrade') ):
    function    wpestate_show_stripe_form_upgrade($stripe_class,$post_id,$price_submission,$price_featured_submission){
        ...
        <div class="stripe_simple upgrade_stripe">
            ...
            data-panel-label="'.esc_html__( 'Set as Featured','wprentals').'"
            data-label="'.esc_html__( 'Set as Featured','wprentals').'"
            data-description="'.esc_html__( ' Featured Payment','wprentals').'">
    ...
    function wpestate_show_stripe_form_per_listing($stripe_class,$post_id,$price_submission,$price_featured_submission){
        ...
        <form action="'.$processor_link.'" method="post" id="stripe_form_simple">
            <div class="stripe_simple">
                ...
                data-description="'.esc_html__( 'Submission Payment','wprentals').'">
                </script>
            </div>
            <input type="hidden" id="propid" name="propid" value="'.$post_id.'">
            <input type="hidden" id="submission_pay" name="submission_pay" value="1">
        ...
        <form action="'.$processor_link.'" method="post" id="stripe_form_featured">
            <div class="stripe_simple">
                ...
                data-currency="'.$submission_curency_status.'"
                data-label="'.esc_html__( 'Pay with Credit Card','wprentals').'"
                data-description="'.esc_html__( 'Submission & Featured Payment','wprentals').'">
                </script>
            </div>
            <input type="hidden" id="propid" name="propid" value="'.$post_id.'">
            <input type="hidden" id="submission_pay" name="submission_pay" value="1">
            ...
            while($pack_selection->have_posts() ){
                 $pack_selection->the_post();
                        ...
                        <div class="stripe_buttons '.$visible_stripe.' stripe_member" id="'.  sanitize_title($title).'">
                            ...
                            data-currency="'.$submission_curency_status.'"
                            data-label="'.esc_html__( 'Pay with Credit Card','wprentals').'"
                            data-description="'.$title.' '.esc_html__( 'Package Payment','wprentals').'">
                            </script>
                        </div>';         
            }
            wp_reset_query();
...
if( !function_exists('wpestate_get_rooms_dropdown') ):
    function wpestate_get_rooms_dropdown(){
        ...
        for($i=2;$i<15;$i++){
            $select_area_list .=   '<li role="presentation" data-value="'. $i.'">'. $i.' '.esc_html__( 'rooms','wprentals').'</li>';
        }
...
if( !function_exists('wpestate_get_bedrooms_dropdown') ):
    function wpestate_get_bedrooms_dropdown(){
        ...
        for($i=2;$i<15;$i++){
            $select_area_list .=   '<li role="presentation" data-value="'. $i.'">'. $i.' '.esc_html__( 'bedrooms','wprentals').'</li>';
        }
...
if( !function_exists('wpestate_get_baths_dropdown') ):
    function wpestate_get_baths_dropdown(){
        ...
        for($i=2;$i<15;$i++){
            $select_area_list .=   '<li role="presentation" data-value="'. $i.'">'. $i.' '.esc_html__( 'baths','wprentals').'</li>';
        }
...
function kriesi_pagination_ajax($pages = '', $range = 2,$paged,$where){  
    ...
    if(1 != $pages)
    {
        ...
        echo "<li class=\"roundleft\"><a href='".get_pagenum_link($paged - 1)."' data-future='".$prev_page."'><i class=\"fas fa-chevron-left\"></i></a></li>";
        ...
        if ( ($paged +1) > $pages){
           $prev_page= get_pagenum_link($paged );
            echo "<li class=\"roundright\"><a href='".$prev_page."' data-future='".$paged."'><i class=\"fas fa-chevron-right\"></i></a><li>"; 
        }else{
            $prev_page= get_pagenum_link($paged + 1);
            echo "<li class=\"roundright\"><a href='".$prev_page."' data-future='".($paged+1)."'><i class=\"fas fa-chevron-right\"></i></a><li>"; 
        }
...
if( !function_exists('wpestate_show_extended_search') ):
    function wpestate_show_extended_search($tip){
        ...
        foreach ($feature_list_array as $key=>$checker) {
            $data= wprentals_prepare_non_latin($checker,$checker);
            $label_array[ $data['key'] ]=$data['label'];
        }
        ...
        print '<span id="adv_extended_close_adv"><i class="fas fa-times"></i></span>';
                ...
                foreach($advanced_exteded as $checker => $value){
                    ...
                    if (function_exists('icl_translate') ){
                        $label     =   icl_translate('wprentals','wp_estate_property_custom_amm_'.$label, $label ) ;                                      
                    }
...
if( !function_exists('wpestate_get_action_select_list') ):
    ...
    function wpestate_get_action_select_list($args){
        ...
        $categ_select_list  =   get_transient('wpestate_get_action_select_list_simple'.$transient_appendix);
        if($categ_select_list===false){
            ...
            set_transient('wpestate_get_action_select_list_simple'.$transient_appendix,$categ_select_list,4*60*60);
...
if( !function_exists('wpestate_get_category_select_list') ):
     ...
     function wpestate_get_category_select_list($args){
        $transient_appendix =   '';
        $transient_appendix =   wpestate_add_language_currency_cache($transient_appendix,1);
        $categ_select_list  =   get_transient('wpestate_get_category_select_list_simple'.$transient_appendix);
        ...
        if($categ_select_list===false){
            ...
            set_transient('wpestate_get_category_select_list_simple'.$transient_appendix,$categ_select_list,4*60*60);
...
if( !function_exists('wpestate_get_city_select_list') ):
    ...
    function wpestate_get_city_select_list($args){
        $transient_appendix =   '';
        $transient_appendix =   wpestate_add_language_currency_cache($transient_appendix,1);
        $categ_select_list = get_transient('wpestate_get_city_select_list_simple'.$transient_appendix);
        if($categ_select_list===false){
            ...
            set_transient('wpestate_get_city_select_list_simple'.$transient_appendix,$categ_select_list,4*60*60);
...
if( !function_exists('wpestate_get_area_select_list') ):
    ...
    function wpestate_get_area_select_list($args){
        $transient_appendix =   '';
        $transient_appendix =   wpestate_add_language_currency_cache($transient_appendix,1);
        $categ_select_list = get_transient('wpestate_get_area_select_list_simple'.$transient_appendix);
        ...
        if($categ_select_list===false){
            ...
            set_transient('wpestate_get_area_select_list_simple'.$transient_appendix,$categ_select_list,4*60*60);
...
function wpestate_return_all_fields($is_mandatory=0){
 ...
 //$redux_data = get_option( 'wprentals_admin' );
    ...
    if( !empty($custom_fields)){  
        while($i< count( $custom_fields['add_field_name'] ) ){
//            $name               =   stripslashes($custom_fields['add_field_name'][$i]);
//            $slug               =   str_replace(' ','-',$name);
//            $label              =  stripslashes( $custom_fields['add_field_label'][$i] );
//           
...
//        $post_var_name      =   htmlspecialchars ( $post_var_name ,ENT_QUOTES);
//            
//            
//            
        ...
        $data= wprentals_prepare_non_latin($checker,$checker);
        $all_submission_fields[ $data['key'] ]=$data['label'];
       // $all_submission_fields[wpestate_limit45($post_var_name)]=trim($checker);     
    }
