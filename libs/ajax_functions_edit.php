...
//                $has_errors=true;
//                $errors[]=esc_html__( 'Please chose a room type for your listing','wprentals');
//            }
//        }
            ...
            <div class="create_invoice_form">
                   ...
                   <div class="invoice_table">
                            ...
                            if($booking_array['price_per_guest_from_one']==1){
                                ...
                                if($booking_array['cover_weekend']){
                                    ...
                                    if($booking_array['has_custom']){
                                        ...
                                        $new_price_to_show=$price_show.' '.wpestate_show_labels('per night',$rental_type,$booking_type);
                                    }
                                }
                    ...
                    if( floatval($depozit)==0){
                        print '<span id="confirm_zero_instant_booking"   data-propid="'.$property_id.'" data-bookid="'.$booking_id.'" data-invoiceid="'.$invoice_id.'">'.esc_html__( 'Confirm Booking - Nothing To Pay','wprentals').'</span>';
                        ...
                        if ( $is_paypal_live=='yes'){
                            print '<span id="paypal_booking" data-deposit="'.$depozit.'"  data-propid="'.$property_id.'" data-bookid="'.$booking_id.'" data-invoiceid="'.$invoice_id.'">'.esc_html__( 'Pay with Paypal','wprentals').'</span>';
                        }
                        $enable_direct_pay      =   esc_html ( wprentals_get_option('wp_estate_enable_direct_pay','') );

                        if ( $enable_direct_pay=='yes'){
                          //  print '<span id="direct_pay_booking" data-propid="'.$property_id.'" data-bookid="'.$booking_id.'" data-invoiceid="'.$invoice_id.'">'.esc_html__( 'Wire Transfer','wprentals').'</span>';
                        }
                    }
                  print'
                  </div>
            ...
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    foreach($feature_list_array as $key => $value){
                        ...
                        $moving_array[] =   $post_var_name.'-'.$feature_value;
                        $i++;
                    }
        ...
        if( isset( $_POST['listing_edit'] ) ) {
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    $api_update_details['local_booking_type']                       =   $book_type;
